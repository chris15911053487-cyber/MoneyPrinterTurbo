...
def _combine_videos_by_segments(
    ...
    """
    按句子分段合成视频，每个句子的素材只播放该句的朗读时长。

    sentence_segments: [{"videos": [path, ...], "duration": 2.3}, ...]
    """
    ...
    total_target = sum(seg.get("duration", 0) for seg in sentence_segments)
    logger.info(
        f"combining {len(sentence_segments)} sentence segments, "
        f"total target duration: {total_target:.2f}s"
    )

    processed_clips = []
    clip_counter = 1

    for seg_idx, segment in enumerate(sentence_segments):
        seg_videos = segment.get("videos", [])
        target_duration = segment.get("duration", max_clip_duration)
        if target_duration <= 0:
            continue

        seg_elapsed = 0.0
        seg_text = str(segment.get("sentence", ""))[:60]
        logger.debug(
            f"segment {seg_idx}: target={target_duration:.2f}s, "
            f"videos={len(seg_videos)}, text='{seg_text}'"
        )

        if not seg_videos:
            logger.warning(f"segment {seg_idx}: no videos, skipping")
            continue

        # Create subclips from this segment's videos
        seg_subclips = []
        for video_path in seg_videos:
            try:
                ...
                logger.warning(f"segment {seg_idx}: cannot open {video_path}: {e}")
                continue
            ...
            while start_time < clip_duration:
                ...
                start_time = end_time
                # Only take one subclip per video for segments to keep variety
                break

        # Process subclips until this segment's duration target is met
        for subclipped_item in seg_subclips:
            if seg_elapsed >= target_duration:
                break
            ...
            try:
                ...
                # 裁剪到不超出该句子的剩余时长
                remaining = target_duration - seg_elapsed
                if clip.duration > remaining:
                    clip = clip.subclipped(0, max(0.1, remaining))

                clip_file = f"{output_dir}/temp-seg{seg_idx}-clip-{clip_counter}.mp4"
                ...
                seg_elapsed += clip_duration_saved
                clip_counter += 1

            except Exception as e:
                logger.error(
                    f"segment {seg_idx} clip processing failed: {str(e)}"
                )

        if seg_elapsed < target_duration:
            logger.debug(
                f"segment {seg_idx}: covered {seg_elapsed:.2f}/{target_duration:.2f}s"
            )

    # Loop processed clips to meet total target duration (safety margin)
    total_elapsed = sum(c.duration for c in processed_clips)
    required = _get_required_video_duration(total_target)
    if total_elapsed < required and processed_clips:
        logger.info(
            f"looping clips to fill {required:.2f}s (have {total_elapsed:.2f}s)"
        )
        base_clips = processed_clips.copy()
        for clip in itertools.cycle(base_clips):
            if total_elapsed >= required:
                break
            processed_clips.append(clip)
            total_elapsed += clip.duration
...
def combine_videos(
    ...
    threads: int = 2,
    clip_speed: float = 1.0,
    sentence_segments: list[dict] | None = None,
) -> str:
    """
    合成视频素材片段。

    当提供 sentence_segments 时，按句子的精确时长分配素材：
    - sentence_segments: [{"videos": [...], "duration": 2.3}, ...]
    - 每个 segment 的视频总时长会被裁剪到恰好等于该句的朗读时长，
      确保画面切换和字幕同步。
    """
    if sentence_segments:
        return _combine_videos_by_segments(
